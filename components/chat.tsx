...
export function Chat({
  ...
  const { messages, handleSubmit, input, setInput, append, isLoading } = useChat({
    ...
    onFinish: () => {
      ...
      selectedFileIds.forEach((fid) => params.append("s", String(fid)));
      const qs = params.toString();
      window.history.replaceState({}, "", `/${id}${qs ? `?${qs}` : ""}`);
    },
  });
  ...
  const onSaveMessage = async (index: number) => {
    ...
    const filename = `${(title || "Message").replace(/[^a-zA-Z0-9\s]/g, "").trim()} ${Date.now()}.md`;
    await uploadFile(filename, body, title || null);
  };
