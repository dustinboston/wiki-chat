...
import { computeHighlightSpans, HighlightedBody } from '@/components/note-page';
...
describe('computeHighlightSpans', () => {
	it('finds a single match', () => {
		const spans = computeHighlightSpans('the quick brown lorem ipsum jumps', [
	...
	it('matches case-insensitively', () => {
		const spans = computeHighlightSpans('Hello WORLD', [{ quotedText: 'world', noteFileId: 2 }]);
		expect(spans).toHaveLength(1);
		expect(spans[0].noteFileId).toBe(2);
	});

	it('finds multiple non-overlapping matches of the same needle', () => {
		const spans = computeHighlightSpans('foo bar foo baz foo', [
	...
	it('drops highlights that overlap an earlier span (first-win)', () => {
		const spans = computeHighlightSpans('abcdefghij', [
	...
	it('skips a highlight whose text is not in the body', () => {
		const spans = computeHighlightSpans('hello world', [
...
describe('HighlightedBody', () => {
	it('renders plain text when no highlights', () => {
		const { container } = render(
			<HighlightedBody content="the quick brown fox" highlights={[]} onHighlightClick={noop} />,
		);
