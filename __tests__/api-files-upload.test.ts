...
describe('POST /api/files/upload auth', () => {
	it('returns 401 when not authenticated', async () => {
		...
		const response = await upload(jsonRequest(
			'http://localhost/api/files/upload?filename=note.md&title=N',
			{content: 'hi'},
		));
...
describe('POST /api/files/upload parentFileId', () => {
	it('returns 404 when parent file missing', async () => {
		...
		const response = await upload(jsonRequest(
			'http://localhost/api/files/upload?filename=note.md&title=N&sourceType=manual',
			{content: 'hi', parentFileId: 7},
		));
	...
	it('returns 403 when parent file belongs to another user', async () => {
		...
		mockGetFileById.mockResolvedValue({
			id: 7, pathname: 'p.md', title: null, userEmail: 'other@b.com', sourceType: 'upload', createdAt: new Date(),
		});
		const response = await upload(jsonRequest(
			'http://localhost/api/files/upload?filename=note.md&title=N&sourceType=manual',
			{content: 'hi', parentFileId: 7},
	...
	it('anchors to first chunk when only parentFileId is provided', async () => {
		mockAuth.mockResolvedValue(mockSession('a@b.com'));
		mockGetFileById.mockResolvedValue({
			id: 7, pathname: 'p.md', title: null, userEmail: 'a@b.com', sourceType: 'upload', createdAt: new Date(),
		});
		mockGetChunksByFileIds.mockResolvedValue([
			{
				id: '7/0', fileId: 7, content: 'First chunk text', embedding: [],
			},
			{
				id: '7/1', fileId: 7, content: 'Second chunk text', embedding: [],
			},
		]);
		const response = await upload(jsonRequest(
			'http://localhost/api/files/upload?filename=note.md&title=N&sourceType=manual',
			{content: 'my note', parentFileId: 7},
		));
	...
	it('anchors to matching chunks for quotedText substring match', async () => {
		...
		mockGetChunksByFileIds.mockResolvedValue([
			...
			{
				id: '7/1', fileId: 7, content: 'The quick brown fox jumps over', embedding: [],
			},
			{
				id: '7/2', fileId: 7, content: 'Another unrelated section', embedding: [],
			},
		]);
		const response = await upload(jsonRequest(
			'http://localhost/api/files/upload?filename=note.md&title=N&sourceType=manual',
			{content: 'my note', parentFileId: 7, quotedText: 'The quick brown fox'},
		));
		expect(response.status).toBe(200);
		const call = getLastCreateFileCall();
	...
	it('falls back to first chunk when quotedText has no match', async () => {
		...
		const response = await upload(jsonRequest(
			'http://localhost/api/files/upload?filename=note.md&title=N&sourceType=manual',
			{content: 'my note', parentFileId: 7, quotedText: 'something entirely unfindable'},
		));
		expect(response.status).toBe(200);
		const call = getLastCreateFileCall();
		expect(call.sourceChunks).toEqual([
	...
	it('ignores client-sent sourceChunks when parentFileId is present', async () => {
		...
		mockGetChunksByFileIds.mockResolvedValue([
			{
				id: '7/0', fileId: 7, content: 'Body', embedding: [],
			},
		]);
		const response = await upload(jsonRequest(
			'http://localhost/api/files/upload?filename=note.md&title=N&sourceType=manual',
