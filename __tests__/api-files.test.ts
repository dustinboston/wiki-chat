...
const { mockAuth, mockListFiles, mockGetFileContent, mockDeleteFile, mockReplaceFileContent } =
	vi.hoisted(() => ({
		mockAuth: vi.fn(),
		mockListFiles: vi.fn(),
...
describe('GET /api/files/content', () => {
	...
	it('truncates content exceeding 5000 words', async () => {
		...
		const longContent = Array.from({ length: 5500 }, (_unused, index) => `word${index}`).join(' ');
		mockGetFileContent.mockResolvedValue({
			file: {
				id: 1,
				pathname: 'doc.pdf',
	...
	function patchRequest(id: string | null, body: unknown): Request {
		const url =
			id === null
				...
				: `http://localhost/api/files/content?id=${id}`;
